create_sa_data <- function(pred_dates) {
    ...
    # To do this, we select weekly dates from the time periods surveyed
    ...
    pred_data <- mutate(pred_data,
                        time = as.integer(difftime(date, as.POSIXct(paste0(as.character(winter), "-12-31")), units="days")),
