...
coei_occ_mad_m <- NanSound_plot(data=coei_final, z = "occup_mad_m", hotspot=2,
                              legend.title = "Occupancy\n(MAD/median)")

# SCOT
scot_occ <- NanSound_plot(data=scot_final, z = "occup", hotspot=2,
                          legend.title = "Occupancy\n(median)", label = "SCOT")
scot_occ_mad_m <- NanSound_plot(data=scot_final, z = "occup_mad_m", hotspot=2,
                              legend.title = "Occupancy\n(MAD/median)")

# LTDU
ltdu_occ <- NanSound_plot(data=ltdu_final, z = "occup", hotspot=2, label = "LTDU",
                         legend.title = "Occupancy\n(median)")
ltdu_occ_mad_m <- NanSound_plot(data=ltdu_final, z = "occup_mad_m", hotspot=2,
                              legend.title = "Occupancy\n(MAD/median)")

# Assemble them
png(file = "./Figures/occupancy_maps.png", width = 18.5, height = 10, units = "in", res = 600)
grid.arrange(do.call(arrangeGrob, list(coei_occ, coei_occ_mad_m, nrow=2)),
             do.call(arrangeGrob, c(blank_plots, list(nrow=2))),
             do.call(arrangeGrob, list(scot_occ, scot_occ_mad_m, nrow=2)),
             do.call(arrangeGrob, c(blank_plots, list(nrow=2))),
             do.call(arrangeGrob, list(ltdu_occ, ltdu_occ_mad_m, nrow=2)),
