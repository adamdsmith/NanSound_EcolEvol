...
coei_lt_abund_mad <- NanSound_plot(data=coei_final, z = "lt_abund_mad", type = "count", hotspot=2,
...
scot_lt_abund_mad <- NanSound_plot(data=scot_final, z = "lt_abund_mad", type = "count", hotspot=2,
                                  legend.title = "Abundance\n(MAD)", plotwind = TRUE)

# LTDU
ltdu_lt_abund <- NanSound_plot(data=ltdu_final, z = "lt_abund", type = "count", hotspot=2,
                              label = "LTDU", legend.title = "Abundance\n(median)", plotwind = TRUE)
ltdu_lt_abund_mad <- NanSound_plot(data=ltdu_final, z = "lt_abund_mad", type = "count", hotspot=2,
                                  legend.title = "Abundance\n(MAD)", plotwind = TRUE)

# Assemble them
png(file = "./Figures/overall_abundance_maps.png", width = 18.5, height = 10, 
    units = "in", res = 600)
grid.arrange(do.call(arrangeGrob, list(coei_lt_abund, coei_lt_abund_mad, nrow=2)),
             do.call(arrangeGrob, c(blank_plots, list(nrow=2))),
             do.call(arrangeGrob, list(scot_lt_abund, scot_lt_abund_mad, nrow=2)),
             do.call(arrangeGrob, c(blank_plots, list(nrow=2))),
             do.call(arrangeGrob, list(ltdu_lt_abund, ltdu_lt_abund_mad, nrow=2)),
