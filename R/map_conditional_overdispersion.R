...
coei_c_overd_mad_m <- NanSound_plot(data=coei_final, z = "c_overd_mad_m", type = "count", hotspot=2,
...
scot_c_overd_mad_m <- NanSound_plot(data=scot_final, z = "c_overd_mad_m", type = "count", hotspot=2,
                                 legend.title = "Conditional\noverdispersion\n(MAD/median)")

# LTDU
ltdu_c_overd <- NanSound_plot(data=ltdu_final, z = "c_overd", hotspot=2, type = "count", 
                             label = "LTDU", legend.title = "Conditional\noverdispersion\n(median)")
ltdu_c_overd_mad_m <- NanSound_plot(data=ltdu_final, z = "c_overd_mad_m", type = "count", hotspot=2,
                                 legend.title = "Conditional\noverdispersion\n(MAD/median)")

# Assemble them
png(file = "./Figures/conditional_overdispersion_maps.png", width = 18.5, height = 10, 
    units = "in", res = 600)
grid.arrange(do.call(arrangeGrob, list(coei_c_overd,coei_c_overd_mad_m, nrow=2)),
             do.call(arrangeGrob, c(blank_plots, list(nrow=2))),
             do.call(arrangeGrob, list(scot_c_overd, scot_c_overd_mad_m, nrow=2)),
             do.call(arrangeGrob, c(blank_plots, list(nrow=2))),
             do.call(arrangeGrob, list(ltdu_c_overd, ltdu_c_overd_mad_m, nrow=2)),
