...
coei_c_abund_mad <- NanSound_plot(data=coei_final, z = "c_abund_mad", type = "count", hotspot=2,
...
scot_c_abund_mad <- NanSound_plot(data=scot_final, z = "c_abund_mad", type = "count", hotspot=2,
                                 legend.title = "Conditional\nabundance\n(MAD)")

# LTDU
ltdu_c_abund <- NanSound_plot(data=ltdu_final, z = "c_abund", hotspot=2, type = "count", 
                             label = "LTDU", legend.title = "Conditional\nabundance\n(median)")
ltdu_c_abund_mad <- NanSound_plot(data=ltdu_final, z = "c_abund_mad", type = "count", hotspot=2,
                                 legend.title = "Conditional\nabundance\n(MAD)")

# Assemble them
png(file = "./Figures/conditional_abundance_maps.png", width = 18.5, height = 10, 
    units = "in", res = 600)
grid.arrange(do.call(arrangeGrob, list(coei_c_abund,coei_c_abund_mad, nrow=2)),
             do.call(arrangeGrob, c(blank_plots, list(nrow=2))),
             do.call(arrangeGrob, list(scot_c_abund, scot_c_abund_mad, nrow=2)),
             do.call(arrangeGrob, c(blank_plots, list(nrow=2))),
             do.call(arrangeGrob, list(ltdu_c_abund, ltdu_c_abund_mad, nrow=2)),
