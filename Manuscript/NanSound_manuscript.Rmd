...
**Sounds worthwhile to me; I've taken a first crack at it but please edit as you see fit.  Some of this might be unnecessary given the paragraph in the introduction.**

To model sea duck occpancy, we used a logistic generalized additive model [GAM; @Hastie1990; @Wood2006] that flexibly accommodated potential nonlinearities between covariates and sea duck occupancy.  We used GAMs for location, scale, and shape (GAMLSS) methods [@Rigby2005] to model the zero-truncated negative binomial count models.  The GAMLSS count models flexibly accommodated potential nonlinearities between covariates and the conditional mean and overdispersion of sea duck abundance. 

We implemented GAMs and GAMLSS in a gradient descent boosting framework [@Mayr2012; @Hofner2015b] that automatically implemented variable and model selection.  **_You mean the general concept of base-learners?_** **I mean to introduce how base-learners relate to, but are distinct from, covariates.**.  Component-wise gradient boosting selects only the single best-fitting base-learner (i.e., functional representaiton of the covariate-response relationship?; @Hofner2014; **maybe just define it briefly paranthetically here?**) in each iteration, which thus integrates into the algorithm intrinsic selection of the most relevant covariates and their functional form (i.e., some base-learners may never be selected).  Moreover, variable selection and prediction accuracy are maximized in gradient boosting frameworks, and model overfitting avoided, by stopping the iterative algorithm prior to convergence to maximum likelihood estimates [i.e., early stopping; **_BH: check if these are best citations_** @Mayr2012; @Maloney2012].  We used 25-fold subsampling to determine the optimal stopping iteration for each model.  Specifically, we randomly drew (without replacement) 25 samples of size $n/2$ from the original data set. We used the selected sample to estimate the model and the balance of the data in each sample to determine the out-of-bag prediction accuracy (empirical risk) measured by the negative loglikelihood of each model; the optimal stopping iteration (*m*~stop~) is the iteration with the lowest average empirical risk.  **_We should add that we need bivariate cross-validation for gamboostLSS models as we need to find the optimum stopping iteration for the mean and overdispersion together._** **I'll leave this to you.** A detailed explanation of the cross-validation scheme is given in @Hofner2015b.
